...
[[https://emacs-tree-sitter.github.io/][tree-sitter]] is built-in since emacs 29. Basically, tree-sitter enhances understanding of source code. To do that, you'll need language grammar libraries.
...
The ~mapc~ line makes the ~treesit-install-language-grammar~ command install the grammar library for each language specified in the ~treesit-language-source-alist~ variable.
